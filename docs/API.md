...
| GET | `/products/{id}/productions` | — | Возвращает все партии производства изделия, от новых к старым. |
| PUT | `/products/{id}/productions/{production_id}` | `quantity?, produced_at?` | Правит ошибочную запись пополнения. При изменении `quantity` — `stock_qty` изделия корректируется на разницу. Остатки материалов не трогает (состав изделия мог измениться с момента пополнения). |
| DELETE | `/products/{id}/productions/{production_id}` | — | Удаляет запись пополнения, вычитая её `quantity` из `stock_qty` изделия. Остатки материалов не трогает. |
